...
impl CourseApplicationService {
    ...
    pub fn get_course_catalog(&self, lang: &str) -> CourseCatalogDto {
        let courses = self.course_repo.find_all(lang);
        let course = courses.into_iter().next().expect("No course data available");
        CourseMapper::to_catalog_dto(course)
    }

    pub fn get_lesson_detail(&self, lesson_id: &str, lang: &str) -> Result<LessonDetailDto, DomainError> {
        let id = LessonId::new(lesson_id);
        self.course_repo
            .find_lesson_by_id(&id, lang)
    ...
    pub fn get_quiz(&self, quiz_id: &str, lang: &str) -> Result<QuizDto, DomainError> {
        let id = QuizId::new(quiz_id);
        self.course_repo
            .find_quiz_by_id(&id, lang)
            .map(|(_, _, _, quiz)| CourseMapper::to_quiz_dto(quiz))
            .ok_or_else(|| DomainError::QuizNotFound(quiz_id.to_string()))
    }

    pub fn get_navigation(&self, lesson_id: &str, lang: &str) -> NavigationDto {
        let id = LessonId::new(lesson_id);
        let prev = self.course_repo.find_prev_lesson(&id, lang);
        let next = self.course_repo.find_next_lesson(&id, lang);
        NavigationDto {
